...
const TeamPage = () => {
  ...
  const [expandedSections, setExpandedSections] = useState({ BE: true, TE: true, SE: true });
  ...
  const handleEditClick = (group) => { setSelectedGroup(group); setModalOpen(true); };
  const closeModal = () => { setModalOpen(false); setSelectedGroup(null); };
  const toggleSection = (section) => setExpandedSections(prev => ({ ...prev, [section]: !prev[section] }));

  const getYearStats = (year) => {
    const groups = groupsByYear[year] || [];
    const totalStudents = groups.reduce((sum, g) => sum + (g.members?.length || 0) + 1, 0);
    return { totalGroups: groups.length, totalStudents };
  };

  const getTotalStats = () => {
    const allGroups = [...(groupsByYear.SE || []), ...(groupsByYear.TE || []), ...(groupsByYear.BE || [])];
    const totalStudents = allGroups.reduce((sum, g) => sum + (g.members?.length || 0) + 1, 0);
    const incompleteGroups = allGroups.filter(g => (g.members?.length || 0) + 1 < g.maxMembers).length;
    const groupsWithoutMentor = allGroups.filter(g => !g.mentor).length;
    return { totalGroups: allGroups.length, totalStudents, incompleteGroups, groupsWithoutMentor };
  };

  const getSectionTitle = (year) => {
    const groups = groupsByYear[year] || [];
    const currentYear = groups[0]?.currentYear || '';
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-slate-50">
                {groups.length === 0 ? (
                  <tr>
                    <td colSpan="7" className="px-6 py-8 text-center text-slate-500">No groups found for {year}</td>
                  </tr>
                ) : (
                  groups.map((group) => {
                    ...
                    const allAvatars = [
                      { initial: (group.admin?.name || group.admin?.email || '?')[0].toUpperCase(), key: 'admin' },
                      ...allMembers.map((m, i) => ({
                        initial: (m.userId?.name || m.userId?.email || '?')[0].toUpperCase(),
                        key: m.userId?._id || i
                      }))
                    ];
                    const visibleAvatars = allAvatars.slice(0, 4);
                    const extraCount = allAvatars.length - 4;
                      ...
                      <tr key={group._id} className="hover:bg-slate-50/50 transition-colors">
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-2">
                            <div className="w-6 h-6 rounded-full bg-blue-100 flex items-center justify-center text-xs text-blue-600 font-bold">
                              {(group.admin?.name || group.admin?.email || '?')[0].toUpperCase()}
                            </div>
                            <span className="text-sm text-slate-600">
                              {group.admin?.name || group.admin?.email?.split('@')[0]}
                            </span>
                          </div>
                        </td>
                        <td className="px-6 py-4">
                          {group.mentor ? (
                            <div className="flex items-center gap-2">
                              <div className="w-6 h-6 rounded-full bg-green-100 flex items-center justify-center text-xs text-green-600 font-bold">
                                {(group.mentor.name || group.mentor.email || '?')[0].toUpperCase()}
                              </div>
                              <span className="text-sm text-slate-600">
                                {group.mentor.name || group.mentor.email?.split('@')[0]}
                              </span>
                            </div>
                          ) : (
                        ...
                        <td className="px-6 py-4">
                          <div className="flex justify-center -space-x-2">
                            {visibleAvatars.map((av) => (
                              <div key={av.key} className="w-7 h-7 rounded-full border-2 border-white bg-blue-100 flex items-center justify-center text-xs font-bold text-blue-700">
                                {av.initial}
                              </div>
                            ))}
                            {extraCount > 0 && (
                        ...
                        <td className="px-6 py-4">
                          <div className="flex justify-center">
                            <span className={`px-3 py-1 rounded-full text-[11px] font-bold ${isFull ? 'bg-emerald-50 text-emerald-500' : 'bg-orange-50 text-orange-400'}`}>
                              {totalMembers}/{group.maxMembers}
                            </span>
                          </div>
                        </td>
                        <td className="px-6 py-4 text-right">
                          <button onClick={() => handleEditClick(group)} className="p-2 text-slate-300 hover:text-indigo-600 transition-colors">
                            <Edit3 size={16} />
                          </button>
                        </td>
                      </tr>
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen flex bg-gray-50">
        <div className="hidden md:flex"><CoordinatorSidebar /></div>
    ...
    <div className="min-h-screen flex bg-gray-50">
      ...
      {sidebarOpen && (
        <div className="fixed inset-0 z-40 flex">
          ...
          <div className="relative z-50 w-64 bg-white shadow-xl"><CoordinatorSidebar /></div>
        </div>
      )}
          ...
          <div className="p-4 md:p-10 font-sans">
            <div className="max-w-[1400px] mx-auto grid grid-cols-1 lg:grid-cols-4 gap-8">
              ...
              <div className="lg:col-span-3">
                <TableSection title={getSectionTitle('BE')} year="BE" isOpen={expandedSections.BE} onToggle={() => toggleSection('BE')} />
                <TableSection title={getSectionTitle('TE')} year="TE" isOpen={expandedSections.TE} onToggle={() => toggleSection('TE')} />
                <TableSection title={getSectionTitle('SE')} year="SE" isOpen={expandedSections.SE} onToggle={() => toggleSection('SE')} />
              </div>
                ...
                <div className="bg-white rounded-[1.5rem] shadow-sm border border-slate-100 p-6 sticky top-10">
                  ...
                  <div className="space-y-4 mb-8 border-t border-slate-50 pt-6">
                    <div className="flex justify-between items-center text-sm">
                      ...
                      <span className="font-bold text-red-500">{totalStats.groupsWithoutMentor}</span>
                    </div>
                    <div className="flex justify-between items-center text-sm">
                  ...
                  <button
                    onClick={fetchGroups}
                    className="w-full py-3 bg-blue-500 hover:bg-blue-600 text-white rounded-xl font-bold text-sm shadow-lg shadow-blue-500/25 transition-all flex items-center justify-center gap-2"
                  >
                    Refresh Data
                  </button>
                </div>
      ...
      <GroupActionModal isOpen={modalOpen} onClose={closeModal} group={selectedGroup} />
    </div>
  );
};
